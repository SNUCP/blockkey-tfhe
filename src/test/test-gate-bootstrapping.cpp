...
int32_t main(int32_t argc, char **argv) {
  ...
  for (int32_t trial = 0; trial < nb_trials; ++trial) {
    ...
    for (int32_t i = nb_samples - 1; i > 0; --i) {
      bootsSparseNAND(test_in + i, test_in + (2 * i), test_in + (2 * i + 1),
                      &keyset->cloud);
    }
    clock_t end = clock();
    cout << "finished bootstrappings NAND tree" << endl;
    cout << "time per bootNAND gate (microsecs)... "
         << (end - begin) / double(nb_samples - 1) << endl;
    ...
    for (int32_t i = nb_samples - 1; i > 0; --i) {
      ...
      if (out != 1 - (mess1 * mess2)) {
        ...
        cout << t32tod(lwePhase(test_in + (2 * i + 1), keyset->lwe_key))
             << endl;
      }
    }
