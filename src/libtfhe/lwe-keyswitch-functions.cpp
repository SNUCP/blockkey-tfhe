 ...
 * compute the error of the KS that has been generated and translate the ks to
recenter the gaussian in 0
*/
void renormalizeKSkey(LweKeySwitchKey *ks, const LweKey *out_key,
                      const int32_t *in_key) {
 ...
 */
void lweCreateKeySwitchKey_fromArray(LweSample ***result, const LweKey *out_key,
                                     const double out_alpha,
                                     const int32_t *in_key, const int32_t n,
                                     const int32_t t, const int32_t basebit) {
 ...
 * @param basebit Log_2 of base
 */
void lweKeySwitchTranslate_fromArray(LweSample *result, const LweSample ***ks,
                                     const LweParams *params, const Torus32 *ai,
                                     const int32_t n, const int32_t t,
...
EXPORT void lweCreateKeySwitchKey_old(LweKeySwitchKey *result,
  ...
  lweCreateKeySwitchKey_fromArray(result->ks, out_key,
                                  out_key->params->alpha_min, in_key->key, n, t,
                                  basebit);
...
*/
EXPORT void lweCreateKeySwitchKey(LweKeySwitchKey *result, const LweKey *in_key,
                                  const LweKey *out_key) {
  const int32_t n = result->n;
  const int32_t t = result->t;
  const int32_t basebit = result->basebit;
  ...
  for (int32_t i = 0; i < n; ++i) {
    for (int32_t j = 0; j < t; ++j) {
      ...
      // term h=0 as trivial encryption of 0 (it will not be used in the
      // KeySwitching)
      ...
      for (int32_t h = 1; h < base; ++h) { // pas le terme en 0
        ...
        */
        ...
        lweSymEncryptWithExternalNoise(&result->ks[i][j][h], mess, noise[index],
                                       alpha, out_key);
        index += 1;
      }
    }
 ...
 */
EXPORT void init_LweKeySwitchKey(LweKeySwitchKey *obj, int32_t n, int32_t t,
                                 int32_t basebit, const LweParams *out_params) {
  const int32_t base = 1 << basebit;
...
EXPORT void free_LweKeySwitchKey(LweKeySwitchKey *ptr) { free(ptr); }
...
EXPORT void init_LweKeySwitchKey_array(int32_t nbelts, LweKeySwitchKey *obj,
                                       int32_t n, int32_t t, int32_t basebit,
...
EXPORT void destroy_LweKeySwitchKey_array(int32_t nbelts,
                                          LweKeySwitchKey *obj) {
  for (int32_t i = 0; i < nbelts; i++) {
...
EXPORT LweKeySwitchKey *new_LweKeySwitchKey(int32_t n, int32_t t,
                                            int32_t basebit,
                                            const LweParams *out_params) {
...
EXPORT LweKeySwitchKey *new_LweKeySwitchKey_array(int32_t nbelts, int32_t n,
                                                  int32_t t, int32_t basebit,
                                                  const LweParams *out_params) {
