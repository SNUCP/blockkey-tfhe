...
EXPORT void init_TGswSample(TGswSample *obj, const TGswParams *params) {
  ...
  TLweSample *all_sample = new_TLweSample_array(
      (k + 1) * l,
      params->tlwe_params); // tous les samples comme un vecteur ligne
  TLweSample **bloc_sample = new TLweSample
      *[k + 1]; // blocs horizontaux (l lignes) de la matrice TGsw
...
EXPORT void tGswSparseKeyGen(TGswKey *result) {
  tLweSparseKeyGen(&result->tlwe_key);
}
...
// Result += mu*H
EXPORT void tGswAddMuH(TGswSample *result, const IntPolynomial *message,
  ...
  for (int32_t bloc = 0; bloc <= k; ++bloc)
    for (int32_t i = 0; i < l; i++) {
      Torus32 *target = result->bloc_sample[bloc][i].a[bloc].coefsT;
...
EXPORT void tGswAddMuIntH(TGswSample *result, const int32_t message,
                          const TGswParams *params) {
  const int32_t k = params->tlwe_params->k;
  const int32_t l = params->l;
  const Torus32 *h = params->h;
...
// Result = tGsw(0)
EXPORT void tGswEncryptZero(TGswSample *result, double alpha,
                            const TGswKey *key) {
...
EXPORT void tGswMulByXaiMinusOne(TGswSample *result, int32_t ai,
                                 const TGswSample *bk,
                                 const TGswParams *params) {
...
#undef INCLUDE_TGSW_MUL_BY_XAI_MINUS_ONE
// mult externe de X^{a_i} par bki
EXPORT void tGswMulByXai(TGswSample *result, int32_t ai, const TGswSample *bk,
                         const TGswParams *params) {
  const TLweParams *par = params->tlwe_params;
  const int32_t kpl = params->kpl;
  for (int32_t i = 0; i < kpl; i++)
    tLweMulByXai(&result->all_sample[i], ai, &bk->all_sample[i], par);
}
#endif
...
// void tGswTLweDecompH(IntPolynomial* result, const TLweSample* sample,const
// TGswParams* params); accum *= sample
EXPORT void tGswExternMulToTLwe(TLweSample *accum, const TGswSample *sample,
                                const TGswParams *params) {
  const TLweParams *par = params->tlwe_params;
 ...
 */
EXPORT void tGswSymEncryptInt(TGswSample *result, const int32_t message,
                              double alpha, const TGswKey *key) {
  tGswEncryptZero(result, alpha, key);
 ...
 */
EXPORT void tGswEncryptB(TGswSample *result, const int32_t message,
                         double alpha, const TGswKey *key) {
  tGswEncryptZero(result, alpha, key);
  if (message == 1)
...
// à revoir
EXPORT void tGswSymDecrypt(IntPolynomial *result, const TGswSample *sample,
                           const TGswKey *key, const int32_t Msize) {
...
// EXPORT void tLweExternMulRLweTo(RLwe* accum, Gsw* a); //  accum = a \odot
// accum

#if defined INCLUDE_ALL || defined INCLUDE_TGSW_TLWE_DECOMP_H
#undef INCLUDE_TGSW_TLWE_DECOMP_H
// fonction de decomposition
EXPORT void tGswTLweDecompH(IntPolynomial *result, const TLweSample *sample,
                            const TGswParams *params) {
  const int32_t k = params->tlwe_params->k;
  const int32_t l = params->l;
...
EXPORT void Torus32PolynomialDecompH_old(IntPolynomial *result,
...
EXPORT void tGswTorus32PolynomialDecompH(IntPolynomial *result,
                                         const TorusPolynomial *sample,
                                         const TGswParams *params) {
  const int32_t N = params->tlwe_params->N;
  const int32_t l = params->l;
...
//#define __AVX2__ //(to test)
...
#else
  {
    ...
    __asm__ __volatile__("vpbroadcastd (%2),%%ymm0\n"
                         "0:\n"
                         "vmovdqu (%0),%%ymm3\n"
  ...
  for (int32_t p = 0; p < l; ++p) {
    ...
    __asm__ __volatile__("vpbroadcastd (%4),%%ymm0\n"
                         "vpbroadcastd (%5),%%ymm1\n"
                         "vmovd (%3),%%xmm2\n"
                         "1:\n"
                         "vmovdqu (%1),%%ymm3\n"
                         ...
                         "cmpq %2,%1\n"
                         "jb 1b\n"
                         : "=r"(dst), "=r"(sit), "=r"(send), "=r"(decal_addr),
                           "=r"(halfBg_addr), "=r"(maskMod_addr)
                         : "0"(dst), "1"(sit), "2"(send), "3"(decal_addr),
                           "4"(halfBg_addr), "5"(maskMod_addr)
    ...
    for (int32_t j = 0; j < N; ++j)
    {
        ...
        fprintf(stderr,
    "j=%d,buf[j]=%u,decal=%u,mask=%u,halfbg=%d,res_p[j]=%d\n",j,buf[j],decal,maskMod,halfBg,res_p[j]);
        abort();
        }
    }*/
...
// result = a*b
EXPORT void tGswExternProduct(TLweSample *result, const TGswSample *a,
                              const TLweSample *b, const TGswParams *params) {
 ...
 * result = (0,mu)
 */
EXPORT void tGswNoiselessTrivial(TGswSample *result, const IntPolynomial *mu,
                                 const TGswParams *params) {
