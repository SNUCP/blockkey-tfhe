...
EXPORT void init_LweBootstrappingKeyFFT(LweBootstrappingKeyFFT *obj,
  ...
  new (obj) LweBootstrappingKeyFFT(in_out_params, bk_params, accum_params,
                                   extract_params, bkFFT, ks);
}
#endif
...
EXPORT void destroy_LweBootstrappingKeyFFT(LweBootstrappingKeyFFT *obj) {
  ...
  delete_TGswSampleFFT_array(obj->in_out_params->n,
                             (TGswSampleFFT *)obj->bkFFT);

  obj->~LweBootstrappingKeyFFT();
}

void tfhe_MuxRotate_FFT(TLweSample *result, const TLweSample *accum,
                        const TGswSampleFFT *bki, const int32_t barai,
 ...
 */
EXPORT void tfhe_blindRotate_FFT(TLweSample *accum, const TGswSampleFFT *bkFFT,
...
EXPORT void tfhe_bootstrap_woKS_FFT(LweSample *result,
  ...
  tfhe_blindRotateAndExtract_FFT(result, testvect, bk->bkFFT, barb, bara, n,
                                 bk_params);
...
EXPORT LweBootstrappingKeyFFT *
alloc_LweBootstrappingKeyFFT_array(int32_t nbelts) {
  return (LweBootstrappingKeyFFT *)malloc(nbelts *
                                          sizeof(LweBootstrappingKeyFFT));
}
...
EXPORT void free_LweBootstrappingKeyFFT_array(int32_t nbelts,
                                              LweBootstrappingKeyFFT *ptr) {
  free(ptr);
}

// initialize the key structure

EXPORT void init_LweBootstrappingKeyFFT_array(int32_t nbelts,
                                              LweBootstrappingKeyFFT *obj,
                                              const LweBootstrappingKey *bk) {
...
EXPORT void destroy_LweBootstrappingKeyFFT_array(int32_t nbelts,
                                                 LweBootstrappingKeyFFT *obj) {
  for (int32_t i = 0; i < nbelts; i++) {
...
EXPORT LweBootstrappingKeyFFT *
new_LweBootstrappingKeyFFT(const LweBootstrappingKey *bk) {
  LweBootstrappingKeyFFT *obj = alloc_LweBootstrappingKeyFFT();
  init_LweBootstrappingKeyFFT(obj, bk);
  return obj;
}
EXPORT LweBootstrappingKeyFFT *
new_LweBootstrappingKeyFFT_array(int32_t nbelts,
                                 const LweBootstrappingKey *bk) {
...
EXPORT void delete_LweBootstrappingKeyFFT_array(int32_t nbelts,
                                                LweBootstrappingKeyFFT *obj) {
