...
default_80bit_gate_bootstrapping_parameters() {
  ...
  // lambda = **80-bit security** (w.r.t bkz-sieve model, + hybrid attack model)
  ...
  return new TFheGateBootstrappingParameterSet(ks_length, ks_basebit, params_in,
                                               params_bk);
}
...
new_default_gate_bootstrapping_parameters(int32_t minimum_lambda) {
  if (minimum_lambda > 128)
    die_dramatically("Sorry, for now, the parameters are only implemented for "
                     "80bit and 128bit of security!");
  ...
  die_dramatically("the requested security parameter must be positive "
                   "(currently, 80 and 128-bits are supported)");
  abort();
}
