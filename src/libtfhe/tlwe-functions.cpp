...
EXPORT void tLweNoiselessTrivialT(TLweSample *result, const Torus32 mu,
                                  const TLweParams *params) {
  const int32_t k = params->k;
...
EXPORT void tLweAddMulRTo(TLweSample *result, const IntPolynomial *p,
  ...
  result->current_variance +=
      intPolynomialNormSq2(p) * sample->current_variance;
}
...
EXPORT void destroy_TLweKey(TLweKey *obj) { (obj)->~TLweKey(); }

EXPORT void init_TLweSample(TLweSample *obj, const TLweParams *params) {
  new (obj) TLweSample(params);
}
EXPORT void destroy_TLweSample(TLweSample *obj) { (obj)->~TLweSample(); }
