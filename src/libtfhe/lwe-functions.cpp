...
/**
 * This function generates a random sparse Lwe key for the given parameters.
 * The Lwe key for the result must be allocated and initialized
 * (this means that the parameters are already in the result)
 */
EXPORT void lweSparseKeyGen(LweKey *result, int32_t hw) {
  ...
  for (int32_t i = 0; i < hw; i++) {
    ...
    int32_t idx = distribution(generator);

    result->key[i * d + idx] = 1;
  }
}
 ...
 * (this means that the parameters are already in the result)
 */
EXPORT void lweSymEncrypt(LweSample *result, Torus32 message, double alpha,
                          const LweKey *key) {
 ...
 */
EXPORT void lweSymEncryptWithExternalNoise(LweSample *result, Torus32 message,
                                           double noise, double alpha,
                                           const LweKey *key) {
  const int32_t n = key->params->n;
 ...
 * The constant Msize indicates the message space and is used to approximate the
 * phase
 */
EXPORT Torus32 lweSymDecrypt(const LweSample *sample, const LweKey *key,
                             const int32_t Msize) {
  Torus32 phi;
...
/** result = sample */
EXPORT void lweCopy(LweSample *result, const LweSample *sample,
...
/** result = -sample */
EXPORT void lweNegate(LweSample *result, const LweSample *sample,
                      const LweParams *params) {
  const int32_t n = params->n;
...
/** result = (0,mu) */
EXPORT void lweNoiselessTrivial(LweSample *result, Torus32 mu,
                                const LweParams *params) {
  const int32_t n = params->n;
...
intVecSubTo_avx(int32_t *r, const int32_t *a, int64_t n) {
...
int32_t intVecSubTo_avx_test() {
  ...
  for (int32_t trial = 0; trial < 100; trial++) {
    ...
    for (int32_t i = 0; i < dim; i++)
      if (tst[i] != tst3[i] - tst2[i])
        abort();
    for (int32_t i = dim; i < 1000; i++)
      if (tst[i] != tst3[i])
        abort();
  }
  return tst[0];
...
EXPORT void lweSubTo(LweSample *result, const LweSample *sample,
                     const LweParams *params) {
  const int32_t n = params->n;
...
EXPORT void lweAddMulTo(LweSample *result, int32_t p, const LweSample *sample,
                        const LweParams *params) {
  const int32_t n = params->n;
...
/** result = result - p.sample */
EXPORT void lweSubMulTo(LweSample *result, int32_t p, const LweSample *sample,
                        const LweParams *params) {
  const int32_t n = params->n;
...
// explicit destructor
EXPORT void destroy_LweSample(LweSample *obj) { obj->~LweSample(); }
...
// explicit destructor
EXPORT void destroy_LweKey(LweKey *obj) { obj->~LweKey(); }
